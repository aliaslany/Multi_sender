...
Because a submission specifies its own destinations, `website` sets `Item.destination_overrides` per item instead of using a fixed `target_senders` list — a submission naming only a Telegram chat id is delivered only to Telegram, never to your own default Bale/Rubika/Eitaa channels. By default the post text is sent exactly as the customer wrote it - the nature-quote + channel-links footer from `telegram_relay` is opt-in here (an "add extras" checkbox on the form), since an arbitrary customer's post has no reason to carry your nature content. One thing is never optional: every post submitted through a promo code gets a short MultiSender attribution link appended, regardless of that checkbox. It goes through the Worker's tracked `/r/` redirect, so clicks are counted per wording and a friend who signs up through it can reward the customer whose post they saw (see "Self-serve trials, referrals" in `worker/README.md`).
...
2. Put that URL into `API_BASE` in `docs/config.js` (every page reads it from there).
...
6. Nothing to mint by hand: the wizard issues a limited trial code to anyone on the spot (`POST /trial`), and paid credit is sold as post packs. `worker/README.md` covers issuing a code yourself, publishing prices, and turning on online payments (Zarinpal).
...
**If someone finds your Rubika bot directly and messages it, it now replies** with a link to the site instead of staying silent — a real gap this closed, since a bot that says nothing when messaged is a dead end for anyone who hears about it secondhand. See "Rubika auto-reply" in `worker/README.md` to set it up (two more Worker secrets, one registration call). Telegram can't get the same treatment without breaking `telegram_relay`'s polling (a bot can only use a webhook or polling, not both); Bale has no such conflict if it's ever worth adding there too.
