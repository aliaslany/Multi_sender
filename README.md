...
آن پست از کجا می‌آید کاملاً به شما بستگی دارد و *اختیاری* است — می‌توانید مستقیم از یک فرم پستش کنید، آن را از یک کانال تلگرام رله کنید، یا یک *منبع* خزنده به یک سایت/API دیگر وصل کنید. به‌صورت پیش‌فرض یک منبع [دیوار](https://divar.ir) کاملاً کاربردی همراه ریپازیتوری می‌آید، اما این فقط یکی از چند گزینهٔ موجود است، نه هستهٔ اصلی پروژه — لایهٔ منبع و لایهٔ ارسال‌کننده‌ها از طریق دو رابط سادهٔ برنامه‌نویسی (interface) کاملاً از هم جدا شده‌اند، پس می‌توانید این ریپازیتوری را فورک کنید و به یک منبع دیگر (یا هیچ‌کدام، فقط فرم دستی) وصلش کنید، بدون اینکه لازم باشد منطق ارسال را دست بزنید.
...
- **منبع محتوا کاملاً اختیاری و قابل‌تعویض** — پست دستی از طریق فرم، رله از تلگرام، یا یک خزندهٔ سفارشی؛ به `main.py` فرقی نمی‌کند از کدام منبع می‌آید (به بخش [معماری](#معماری) نگاه کنید).
...
چون هر پست مقصد خودش را مشخص می‌کند، `website‎` به‌جای یک لیست ثابت `target_senders‎`، برای هر آیتم مقدار `Item.destination_overrides‎` را تنظیم می‌کند — پستی که فقط شناسهٔ چت تلگرام را نام می‌برد، فقط به تلگرام تحویل داده می‌شود، هرگز به کانال‌های پیش‌فرض بله/روبیکا/ایتای خودتان. به‌طور پیش‌فرض، متن پست دقیقاً همان چیزی است که مشتری نوشته — فوتر نقل‌قول طبیعت + لینک کانال‌های `telegram_relay‎` اینجا اختیاری است (یک تیک «افزودن موارد اضافه» روی فرم)، چون دلیلی ندارد پست دلخواه یک مشتری، محتوای طبیعت شما را حمل کند. یک چیز هرگز اختیاری نیست: هر پستی که با یک کد تبلیغی ارسال شود، صرف‌نظر از آن تیک، یک لینک انتساب «multiSender» به `@Divarassist‎` هم دریافت می‌کند.

**لینک معرفی در انتهای هر پست:** به انتهای هر پستی که با کد ارسال می‌شود یک لینک کوتاه معرفی MultiSender اضافه می‌شود (این بخش اختیاری نیست). این لینک از مسیر ردیابی‌شدهٔ `/r/` در Worker می‌گذرد؛ به همین دلیل تعداد کلیک هر متن جایگزین شمرده می‌شود و دوستی که از طریق آن ثبت‌نام کند می‌تواند برای صاحب پست اعتبار هدیه بگیرد (به «Self-serve trials, referrals» در `worker/README.md` نگاه کنید).
...
2. همان آدرس را در `API_BASE` فایل `docs/config.js` قرار دهید (همهٔ صفحه‌ها از همان‌جا می‌خوانند).
...
6. نیازی به ساخت دستیِ کد نیست: ویزارد به هر کسی در همان لحظه یک کد آزمایشیِ محدود می‌دهد (`POST /trial`) و اعتبار پولی به‌صورت «بستهٔ پستی» فروخته می‌شود. صدور دستی کد، انتشار قیمت‌ها و فعال‌سازی پرداخت آنلاین (زرین‌پال) در `worker/README.md` آمده است.
...
**اگر کسی مستقیماً ربات روبیکای شما را پیدا کند و به آن پیام بدهد، حالا پاسخ می‌دهد** — به جای سکوت کامل، لینک سایت را برایش می‌فرستد. این یک شکاف واقعی را برطرف می‌کند، چون رباتی که به پیام جواب نمی‌دهد، برای هر کسی که غیرمستقیم دربارهٔ آن شنیده، یک بن‌بست است. برای راه‌اندازی، بخش «پاسخ خودکار روبیکا» در `worker/README.md‎` را ببینید (دو سکرت دیگر روی Worker، به‌علاوهٔ یک فراخوانی ثبت یک‌باره). تلگرام نمی‌تواند همین رفتار را داشته باشد بدون این‌که پولینگ `telegram_relay‎` را خراب کند (یک ربات فقط می‌تواند وب‌هوک یا پولینگ داشته باشد، نه هر دو)؛ بله چنین تداخلی ندارد، اگر روزی آنجا هم لازم شد.
